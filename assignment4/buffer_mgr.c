...
int CountOfWrite = 0; /* number of write page since the BUffer Pool initializes */
int NumOfBufferBool = 0;  /* the size of the buffer pool */
...
void pageFrameReplace(Frame_Data *frame, BM_BufferPool *const bm, BM_PageHandle *const page, int pageNum) {
  ...
  switch (bm->strategy) {
    case RS_FIFO:
      FIFO(bm, frame, NumOfBufferBool);
      break;

    case RS_LRU:
      LRU(bm, frame, NumOfBufferBool);
      break;

    case RS_CLOCK:
      CLOCK(bm, frame, NumOfBufferBool);
      break;
...
RC initBufferPool(BM_BufferPool *const bm, const char *const pageFileName,
  ...
  // set the NumOfBufferBool with the total number of pages in memory or the buffer pool.
  NumOfBufferBool = numPages;

  // init Page Frame
  Frame_Data *page = initPageFrame(NumOfBufferBool);
...
RC shutdownBufferPool(BM_BufferPool *const bm) {
  ...
  int idx;
  for (idx = 0; idx < NumOfBufferBool; idx++) {
...
RC forceFlushPool(BM_BufferPool *const bm) {
  ...
  for (i = 0; i < NumOfBufferBool; i++) {
    if (pf[i].fixCount == 0 && pf[i].dirtyFlag == 1) {
      ...
      // Increase the CountOfWrite which records the number of writes done by the buffer manager.
      CountOfWrite++;
...
RC pinPage(BM_BufferPool *const bm, BM_PageHandle *const page, const PageNumber pageNum) {
  ...
  // If bFull = true, it means that the buffer is full and can replace an
...
PageNumber *getFrameContents(BM_BufferPool *const bm) {
  PageNumber *nums = malloc(sizeof(PageNumber) * NumOfBufferBool);
  Frame_Data *pf = (Frame_Data *) bm->mgmtData;
  int i = 0;
  while (i < NumOfBufferBool) {
    if (pf[i].pageNum != -1) {
      nums[i] = pf[i].pageNum;
    } else {
      nums[i] = NO_PAGE;
...
bool *getDirtyFlags(BM_BufferPool *const bm) {
  bool *Flags = malloc(sizeof(bool) * NumOfBufferBool);
  Frame_Data *pf = (Frame_Data *) bm->mgmtData;
...
int *getFixCounts(BM_BufferPool *const bm) {
  ...
  int *fixCounts = malloc(sizeof(int) * NumOfBufferBool);
  Frame_Data *pf = (Frame_Data *) bm->mgmtData;

  int j = 0;
  while (j < NumOfBufferBool) {
    if (pf[j].fixCount != -1) {
...
void writeBlockWithThreadSafe(BM_BufferPool *const bm, Frame_Data *frame, int Index) {
  ...
  // Increase the CountOfWrite
  // which records the number of writes done by the buffer manager.
  CountOfWrite++;
...
void LRU(BM_BufferPool *const bufferPool, Frame_Data *page, int pageNum) {
  ...
  for (i = 0; i < pageNum; i++) {
    /* Finding page frame which fixCount = 0 */
    if (fr[i].fixCount == 0) {
      lstHitIndex = i;
...
void CLOCK(BM_BufferPool *const bm, Frame_Data *page, int pageNum) {
  ...
  while (true) {
    ...
    if (frame[clockPtr].hitNum == 0) {
      // If page in memory has been modified (dirtyFLag = 1), so write page to disk
      if (frame[clockPtr].dirtyFlag == 1) {
        writeBlockWithThreadSafe(bm, frame, clockPtr);
      }
      /* Setting page frame's data to new page's data */
